...
const AgentConcerns = () => {
  ...
  const agentEmail = auth?.auth.payload.email; // Replace with dynamic value if needed
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <div className="w-full max-w-4xl mx-auto">
        ...
        {concerns.length > 0 ? (
          concerns.map((item) => (
            <Link to={`/agent/concern/${item.concernId}`} key={item.concernId} state={{ concern: item }} className="block">
              <div
              ...
              >
                <div className="flex-grow">
                  ...
                  <p className="text-gray-600">Customer Email: {item.customerEmail}</p>
          ...
          <div className="text-center text-gray-600">You have no concerns assigned as of now.</div>
        )}
      </div>
    </div>
  );
